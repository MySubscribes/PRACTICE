...
indistinguishable from a Point instance, without calling the
Point constructor.*/
...
The function should return an object with type and value properties
 containing the value of the arguments by those names,
  and a third property, named by the sourceProperty option,
...
//    a.type = type;
//    a.value = value;
//    a[options.sourceProperty] = options.sourceValue;
//   return a
// }
...
This is a typical mistake to make in JavaScript.
 We create a number of functions in a loop, and refer
 to an outside variable from these functions.
 All of them will end up referring to the same variable,
 which ends up being incremented to 10. Thus, callbacks[2]
 does not log 2. It logs 10, as do all functions in the array.

Do you know the solution for such situations in ES5?
Does ES6 provide a cleaner solution?*/
// ES6
// var callbacks = []
// for (let i = 0; i < 10; i++) {
...
/*Изменения возможны - т.к в данном случае - account является ссылкой на обьект - а не самим обьектом.
когда мы меняем password - мы изменяем свойство обьекта а не значение самого обьекта.*/

// Чтоб изменений не было используем сеттер и геттер
// const account = {
//   username: "marijn",
//   _password: "xyzzy",
...
// let password = "s3cret" // (*much* more secure)
...
// (say, filter and reduce) to compute the total value of
// the machines in the inventory array.
...
// // let a = [];
// function valueCost(invent){
//   return invent.type === "machine";
// };
...
// // let totalMachineValue  = a.reduce((sum, current) => sum + current.value);
...
function valueCost(invent){
  if(invent.type === 'machine'){
    return true;
  }
    return false;
};

let totalMachineValue = inventory.filter(valueCost).reduce((sum, current) => sum + current.value,
  );
...
The code for this exercise implements a
wrapper for working with sorted arrays.
Its constructor takes a comparison function
that compares two elements and returns a number,

 negative if the first is less than the second,
 zero when they are equal, and positive otherwise
 (similar to what the sort method on arrays expects).

Rewrite the code to use an ES6 class. Then, rewrite
the loop to use the ES6 array method findIndex, which
is like indexOf, but takes a function instead of an element as argument,
and returns the index of the first element for which that
function returns true (or returns -1 if no such element was found).
 For example [1, 2, 3].findIndex(x => x > 1) yields 1.
...
//     if (this.compare(elt, this.content[i]) < 0)
//       break
//   }
//   return i
// }
